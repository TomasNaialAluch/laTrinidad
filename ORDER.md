...
- Los productos de [assets/products/](assets/products/) agrupados por categoría (cortes frescos, chacinados/embutidos, achuras/menudencias, etc. — a definir la categorización final).
...
- Esta info (nombre, categoría, si tiene precio o no) conviene manejarla como datos en `data/` (similar a `data/clients/`), por ejemplo `data/products.json`, para no tener que tocar el HTML cada vez que cambia un precio o se agrega un producto.
...
- [ ] Armar `data/products.json` con el detalle de cada producto.
